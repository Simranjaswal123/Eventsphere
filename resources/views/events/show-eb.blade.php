...
<div class="relative h-72 sm:h-96 overflow-hidden" style="border-bottom: 1px solid rgba(255,255,255,0.07);">
    ...
    <div class="absolute top-4 left-4">
        <span class="es-badge" style="background: rgba(255,102,0,0.2); color: #ff6600; border: 1px solid rgba(255,102,0,0.35); font-size: 0.7rem; padding: 0.25rem 0.75rem;">
            Eventbrite
        </span>
    </div>
</div>
    ...
    <div class="flex flex-col lg:flex-row gap-10">
        ...
        <div class="flex-1 min-w-0">
            ...
            @if($event['genre'] ?? $event['category'] ?? null)
                <p class="text-xs font-bold uppercase tracking-widest mb-3" style="color: #ff6600;">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-px mb-8 rounded-xl overflow-hidden"
                ...
                <div class="p-5" style="background: #111;">
                    ...
                    @elseif($event['price'])
                        <p class="font-semibold text-white text-lg">${{ number_format($event['price'], 0) }}
            ...
            <div class="p-5 rounded-xl mb-8" style="background: rgba(255,255,255,0.03); border: 1px solid rgba(255,255,255,0.07);">
                ...
                <div class="flex items-start gap-3">
                    <svg class="w-5 h-5 shrink-0 mt-0.5" style="color: #ff6600;" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z"/></svg>
                    <div>
                        <p class="font-semibold text-white">{{ $event['location'] }}</p>
        ...
        <div class="lg:w-80 shrink-0">
            <div class="rounded-2xl overflow-hidden sticky top-20"
                ...
                <div class="p-5" style="border-bottom: 1px solid rgba(255,255,255,0.07);">
                    ...
                    @elseif($event['price'])
                        <p class="font-black text-3xl text-white">${{ number_format($event['price'], 0) }}</p>
                    @else
                        <p class="font-semibold text-xl text-[#555]">Price TBD</p>
                    @endif
                </div>
                ...
                <div class="p-5 space-y-3">
                    ...
                    @if($event['external_url'] && $event['external_url'] !== '#')
                        <a href="{{ $event['external_url'] }}" target="_blank" rel="noopener"
                           class="es-btn w-full py-3 text-sm justify-center"
                    ...
                    <hr class="es-divider">
                    <p class="text-xs text-[#333] text-center">Powered by Eventbrite</p>
                </div>
            </div>
        </div>
